...
const ChatText = ({ align = 'left', name = '', content = '' }) => {
  return (
    <>
      {/* Chat Message AI */}
      {align === 'left' && (
        <div className='flex gap-1 my-2 text-gray-600 text-sm flex-1 mr-9'>
          ...
          <p className='leading-relaxed'>
            ...
            <div className='border border-gray-200 px-2 py-1 rounded-2xl rounded-tl-none bg-gray-100'>{content}</div>
          </p>
        </div>
      )}
      {/* User Chat Message */}
      {align === 'right' && (
        <div className='flex gap-1 justify-end my-2 text-gray-600 text-sm flex-1 ml-9'>
          <p className='leading-relaxed'>
            <span className='flex justify-end block font-bold text-gray-700'>{name}</span>
            <div className='border border-gray-200 px-2 py-1 rounded-2xl rounded-tr-none bg-gray-100'>{content}</div>
          </p>
          <span className='relative flex shrink-0 overflow-hidden rounded-full w-8 h-8'>
            <div className='rounded-full bg-gray-100 border p-1'>
      ...
      {isChatOpen && (
        <div className='z-20 fixed bg-white p-5 md:rounded-lg md:border md:border-[#e5e7eb] w-full h-full md:w-[440px] md:h-[634px] bottom-0 right-0 m-0 md:bottom-[calc(4rem+1.5rem)] md:mr-4'>
          ...
          <div className='mx-[-5px] h-[calc(100vh-140px)] md:h-[485px] overflow-auto min-w-[100%]'>
            <ChatText align='left' name='AI' content='Hi, how can I help you today?' />
            <ChatText align='right' name='You' content='fewafef' />
            <ChatText align='left' name='AI' content={`Sorry, I couldn't find any information in the documentation about that. Expect answer to be less accurateI could not find the answer to this in the verified sources.`} />
            <ChatText align='right' name='You' content='fewafef' />
            <ChatText align='left' name='AI' content={`Sorry, I couldn't find any information in the documentation about that. Expect answer to be less accurateI could not find the answer to this in the verified sources.`} />
            <ChatText align='right' name='You' content='fewafef' />
            <ChatText align='left' name='AI' content={`Sorry, I couldn't find any information in the documentation about that. Expect answer to be less accurateI could not find the answer to this in the verified sources.`} />
